...
<button class="flex items-center text-left py-0.5 
  ...
  <div class="flex">{folder.name}</div>
</button>

{#if (isCrossLaunched() ? expandPriority(folder.details.path) : expanded) && folder.children && folder.children?.length}
	<ul class="ml-2.5 px-2 list-none border-l dark:border-gray-300">
		{#each folder.children as entry}
			<li class="pt-1">
				{#if entry.children.length > 0}
					<svelte:self folder={entry} urlPath={urlPath} />
				{:else}
