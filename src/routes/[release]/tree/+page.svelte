<script lang="ts">
	...
	import type {
		...
		YangTreePostMessage,
		YangTreeResponseMessage
	} from '$lib/workers/structure';
	...
	async function loadYangTreeWorker(
		model: string,
		release: string,
		urlOrigin: string,
		searchInput: string,
		...
		const yangTreeMessage: YangTreePostMessage = {
			model,
			release,
			urlOrigin,
			searchInput,
		...
		<div class="px-6 py-7 container mx-auto">
			...
			<div class="flex py-1 items-center space-x-2">
				...
				<PlatformButton
					...
					platOption={platformSelected}
				/>
			</div>
			<PlatformGrid bind:showPlatformFilters bind:supportedPlatforms bind:platformSelected />
			{#if model == 'openconfig'}
				<p class="mt-3 text-yellow-600 dark:text-yellow-400 text-xs">
          ...
          {pastYangTreeArgs === $yangTreeArgs
						? 'bg-green-100 dark:bg-green-900 text-gray-500 dark:text-gray-500 cursor-not-allowed'
						: 'text-white bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800'}"
					disabled={pastYangTreeArgs === $yangTreeArgs}
