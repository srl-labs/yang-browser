...
{:else if $yangPaths.length > 0}
	<Header
		...
		home={true}
		{supportedPlatforms}
		platformSelected={$platSelect}
	/>
	<!-- svelte-ignore a11y-click-events-have-key-events -->
	<!-- svelte-ignore a11y-no-static-element-interactions -->
	<div
		class="min-w-[280px] overflow-x-auto font-nokia-headline-light dark:bg-gray-800 pt-[75px] lg:pt-[85px]"
		...
		<div class="px-6 pt-6 container mx-auto">
			...
			<div class="overflow-x-auto scroll-light dark:scroll-dark">
				<div class="py-2 space-x-2 flex items-center">
					...
					<PlatformButton
						enabled={supportedPlatforms?.length}
						bind:showPlatformFilters
						{platOption}
					/>
			...
			{#if model == 'openconfig'}
				<p class="mt-2 text-yellow-600 dark:text-yellow-400 text-xs">
					Note: The OpenConfig browser does not take into account the SR Linux deviations. Use pyang
					to generate the tree with the deviations applied.
				</p>
			{/if}
			{#if showPlatformFilters}
				<div transition:fade class="flex flex-wrap items-start mt-4 md:space-x-6">
					<div class="rounded-lg border border-gray-200 dark:border-gray-600 w-full md:w-40">
						<p
							class="px-4 py-2 font-nokia-headline text-gray-900 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 rounded-t-lg"
						>
							Platform
						</p>
						<div class="p-2 border-b border-gray-200 dark:border-gray-600">
							<input
								...
								placeholder="Search..."
								class="w-full px-3 py-1 text-sm rounded-lg text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:placeholder-gray-400"
							/>
						</div>
								...
								<li class="w-full">
									<div class="flex items-center px-3">
										<input
											...
											type="radio"
											name="list-radio"
											class="w-4 h-4 cursor-pointer text-blue-600 bg-gray-100 dark:bg-gray-600"
											bind:group={platOption}
											value=""
											on:change={resetFeatSelect}
										/>
										<label
											for="radio-plat-none"
											class="w-full cursor-pointer py-2 ms-2 text-sm {$platSelect === ''
												? 'text-gray-900 dark:text-gray-300'
												: 'text-gray-400 dark:text-gray-500'}">NONE</label
										>
									</div>
								</li>
								{#each $platList as entry, i}
									<li class="w-full border-t border-gray-200 dark:border-gray-600">
										<div class="flex items-center px-3">
											...
											<label
												for="radio-{entry}"
												class="w-full cursor-pointer py-2 ms-2 text-sm {entry === $platSelect
													? 'text-gray-900 dark:text-gray-300'
													: 'text-gray-400 dark:text-gray-500'}">{entry}</label
											>
										</div>
									</li>
								{/each}
					...
					<div
						class="rounded-lg border border-gray-200 dark:border-gray-600 w-full md:w-fit mt-5 md:mt-0"
					>
						<p
							class="px-4 py-2 font-nokia-headline text-gray-900 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 rounded-t-lg"
						>
						...
						<div class="overflow-y-auto max-h-72 scroll-light dark:scroll-dark">
							<ul>
								{#each $featList as entry, i}
									<li
										class="w-full {i == 0 ? '' : 'border-t border-gray-200 dark:border-gray-600'}"
									>
										<div class="flex items-center px-3">
											<input
												id="checkbox-{entry}"
												...
												on:click={(e) => updateFeatDeviate(e, entry)}
												checked={$featFilter.includes(entry) ? true : false}
											/>
											<label
												for="checkbox-{entry}"
												class="w-full cursor-pointer py-2 ms-2 text-sm {$featSelect.includes(entry)
													? 'text-gray-900 dark:text-gray-300'
													: 'text-gray-400 dark:text-gray-500'}">{entry}</label
											>
										</div>
			...
			<div class="overflow-x-auto rounded-t-lg max-w-full mt-2">
				<table class="text-left w-full">
					...
					<thead
						class="text-sm font-nokia-headline text-gray-800 dark:text-gray-300 bg-gray-300 dark:bg-gray-700"
					>
						<tr>
					...
					<tbody>
						{#if $total > 0}
							{#each $paginated as item}
								{@const path = markFilter(
									showPathPrefix ? item['path-with-prefix'] : item.path,
									$searchStore
								)}
								{@const type = markFilter(item.type, $searchStore)}
								<tr
									class="bg-white dark:bg-gray-800 border-b dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600"
									on:click={() => (popupDetail = item)}
								>
									<td class="px-3 py-1.5 font-fira text-[13px] tracking-tight"
										>{item['is-state']}</td
									>
									<td class="px-3 py-1.5 font-fira text-[13px] tracking-tight group"
										><div use:markRender={path}></div></td
									>
									<td class="px-3 py-1.5 font-fira text-[13px] tracking-tight"
										><div use:markRender={type}></div></td
									>
								</tr>
							{/each}
						{:else}
							<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
								<td colspan="3" class="px-3 py-1.5 font-fira text-[13px] text-red-600 text-center"
									>No results found</td
								>
							</tr>
						{/if}
